...
object CoreServiceManager {
    ...
    fun startVServiceFromToggle(context: Context): Boolean {
        if (MmkvManager.getSelectServer().isNullOrEmpty()) {
            context.toast("Пожалуйста, выберите сервер перед запуском.")
            return false
        }
        try {
    ...
    private fun startContextService(context: Context) {
        ...
        val guid = MmkvManager.getSelectServer()
            ?: run {
                ...
                error("Пожалуйста, выберите сервер перед запуском.")
            }

        val config = MmkvManager.decodeServerConfig(guid)
            ?: run {
                LogUtil.e(AppConfig.TAG, "StartCore-Manager: Failed to decode server config")
                error("Некорректная конфигурация сервера")
            }
...
//        if (!result.status) error(result.errorMessage.ifBlank { "Failed to get V2Ray config" })
        ...
        if (config.insecure == true) {
            context.toastError("Небезопасное соединение")
            context.toastError("Небезопасное соединение")
        }

        if (MmkvManager.decodeSettingsBool(AppConfig.PREF_PROXY_SHARING)) {
            context.toast("Общий доступ к прокси включен")
        } else {
            context.toast("Запуск службы VPN...")
        }
