...
class CoreVpnService : VpnService(), ServiceControl {
    ...
    override fun onCreate() {
        ...
        try {
            val protectCallback = libv2ray.VpnServiceProtect { fd ->
                protect(fd.toInt())
            }
            libv2ray.Libv2ray.registerVpnServiceProtect(protectCallback)
            LogUtil.i(AppConfig.TAG, "VpnServiceProtect registered successfully")
        } catch (e: Exception) {
            LogUtil.e(AppConfig.TAG, "Failed to register VpnServiceProtect", e)
        }
    }
